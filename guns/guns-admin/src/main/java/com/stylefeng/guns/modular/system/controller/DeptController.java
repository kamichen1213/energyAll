...
public class DeptController extends BaseController {
    ...
    @RequestMapping(value = "/getDptListByPid", method = RequestMethod.POST)
    @ResponseBody
    public AjaxWarpper getDptListByPid(Integer pid){
        AjaxWarpper result = new AjaxWarpper(AjaxWarpper.SUCCESS);
        List<TreeViewNode> treeViewNodes = new ArrayList<TreeViewNode>();
        if(pid == null){
            result.setStatus(AjaxWarpper.FAILE);
            result.setErrMsg("父节点为空！");
            return result;
        }
        treeViewNodes = deptService.getAllDeptsByPid(pid);
        result.setErrMsg("success");
        result.setData(treeViewNodes);
        return result;
    }
